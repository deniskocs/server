...
- `POST /api/orchestrator/models/actions` — body `{"action":"download"|"start"|"stop"|"delete_model"|"delete_config","configFile":"my-model.env"}` (файл в `CONFIGS_DIR`, допускается `my-model` без суффикса — сервер допишет `.env`). Неизвестный `configFile` — **404**. **`start`/`stop`**: `VLLM_DOCKER=1`, **`HOST_MODELS_PATH`**, клиент `docker`, сокет; иначе **500**. Для **`start`** в `.env` обязан быть **`PORT`**, иначе **400**. **Start** — `docker pull` + `docker run` (`vllm-orchestrated`); в контейнер vLLM **переменные из `*.env` идут как `-e`**, без монтирования `llm-configs`. **Stop** — `docker stop vllm-orchestrated`. Нет `*.env` в `CONFIGS_DIR` — **400**. Ошибки pull/run/не настроен Docker — **500**.
