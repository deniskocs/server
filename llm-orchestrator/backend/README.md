...
- `POST /api/orchestrator/models/actions` — body `{"action":"download"|"start"|"stop"|"delete_model"|"delete_config","configFile":"my-model.env"}` (файл в `CONFIGS_DIR`, допускается `my-model` без суффикса — сервер допишет `.env`). Неизвестный `configFile` — **404**. **`start`/`stop`**: `VLLM_DOCKER=1`, **`HOST_MODELS_PATH`**, клиент `docker`, сокет; иначе **500**. Для **`start`** в `.env` обязан быть **`PORT`**, иначе **400**. **Start** — `docker pull` + `docker run` (имя контейнера: `vllm-orchestrated--<config-stem>--<хвост-модели>`, см. `vllm_container_name` в `vllm_env.py`); в контейнер vLLM **переменные из `*.env` идут как `-e`**, без монтирования `llm-configs`. **Stop** — `docker stop` того же контейнера, что и для данного `configFile`. Нет `*.env` в `CONFIGS_DIR` — **400**. Ошибки pull/run/не настроен Docker — **500**.

Список конфигов: файлы `*.env` в `CONFIGS_DIR` (в Docker: `/configs`). Сиды в репозитории **нет** — пустой каталог даст пустую таблицу; конфиги добавляй через UI/POST или положи `.env` на диск.
...
Пути можно переопределить **inputs** в workflow. В контейнере API: **Docker socket**, **`HOST_MODELS_PATH`** (хостовый путь к весам для `docker run -v …:/models`), **клиент** `docker` (бинарь из `docker:*-cli`, `DOCKER_PATH`), **имя** контейнера vLLM — **`vllm-orchestrated--…`** (уникально на профиль `.env`). Конфиги оркестратора — **`MODELS_DIR` / `CONFIGS_DIR`**, при **Start** переменные из `*.env` уходят в vLLM как **`-e`** (vLLM entrypoint **без** `/llm-configs`). Сокет обязателен для `Start`/`Stop`. `/api/health` возвращает `MODELS_DIR` / `CONFIGS_DIR`, если заданы.
