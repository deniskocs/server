...
Пути можно переопределить **inputs** в workflow. В контейнере API: **Docker socket**, **`HOST_MODELS_PATH`**, **`HOST_LLM_CONFIGS_PATH`**, **клиент** `docker` (в образе — бинарь из официального `docker:*-cli`, по умолчанию **`/usr/bin/docker`**, `DOCKER_PATH`), **имя** контейнера vLLM — **`vllm-orchestrated`**. **`HOST_MODELS_PATH` и `HOST_LLM_CONFIGS_PATH`** — абсолютные пути к каталогам **на той же машине, что и Docker** (там, где `docker run` монтирует веса и `llm-configs`). Каталоги оркестратора `MODELS_DIR` / `CONFIGS_DIR` в контейнере API должны соответствовать этим путям на хосте (те же бинд-монты). Без полного набора переменных и сокета `Start`/`Stop` не сработают. `/api/health` возвращает `MODELS_DIR` / `CONFIGS_DIR`, если заданы.
