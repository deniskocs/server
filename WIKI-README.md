...
- В репозитории: **`secretstore-bitwarden-tzone.yaml`** (проект **tzone**, US API) и **`external-secret-bitwarden-tzone.yaml`**. Для **`remoteRef.key`**, если это **имя** (не UUID), обязателен **`remoteRef.property`** = **project ID** ([правила провайдера](https://external-secrets.io/latest/provider/bitwarden-secrets-manager/)); если **key** — UUID секрета, `property` не нужен.
...
infra/k8s/server/
  ...
  secretstore-bitwarden-tzone.yaml
  external-secret-bitwarden-tzone.yaml
