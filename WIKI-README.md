...
- В репозитории: **`secretstore-bitwarden-tzone.yaml`** (проект **tzone**, US API) и **`external-secret-bitwarden-tzone.yaml`** — в нём заменить **`remoteRef.key: change-me`** на имя или UUID секрета в BSM.
