...
| Корень K8s-стека под Argo `server` | `infra/k8s/` — Kustomize + `helmCharts` |
| Bootstrap Argo CD (Terraform + Application `server`) | `infra/home-cluster/argocd/` — `values.yaml`, `application.yaml` |

Приложение **`server`** в Argo CD тянет **только** `path: infra/k8s`, `targetRevision: main`. Новые манифесты платформы в кластере добавляются в этот каталог и в `kustomization.yaml`.
...
- Локально: `kubectl kustomize infra/k8s --enable-helm` — в `PATH` должен быть **Helm** (Argo CD использует свой при рендере).
...
- Чарт в **`cert-manager/`**: Jetstack **cert-manager** `v1.20.2`, namespace **`cert-manager`**, `includeCRDs: true`, values — `cert-manager/cert-manager-values.yaml`.
...
Подробности кратко: `infra/k8s/README.md` (секция cert-manager).

---

## 5. External Secrets Operator (ESO)

- Чарт в **`external-secrets/`**: **external-secrets** `2.6.0`, release **`external-secrets`**, namespace **`external-secrets`**, `includeCRDs: true`, values — `external-secrets/external-secrets-values.yaml`.
...
- Подchart **`bitwarden-sdk-server`** в `external-secrets/external-secrets-values.yaml`; TLS — **`external-secrets/certificate-bitwarden-*.yaml`**, **`issuer-bitwarden-internal-ca.yaml`** (waves 15→17, Deployment SDK wave 20).
...
- **`ClusterSecretStore` `bitwarden-cluster`** — **server**, `infra/k8s/external-secrets/bitwarden/` (platform; **пока тот же BSM project ID**, что tzone).
...
infra/home-cluster/               # Terraform: bootstrap Argo CD (Helm)
  ...
  argocd/application.yaml         # one-shot Application server
...
Короткий операционный README по каталогу: **`infra/k8s/README.md`**. Расширенная фиксация знаний — **этот файл (`WIKI-README.md`)**.
...
|-----------------|---------|
| Платформа | Terraform ставит Argo CD (`infra/home-cluster/argocd/values.yaml`); Application `server` — `argocd/application.yaml`; деплой `infra/k8s`. |
...
**Что сделано:** в `infra/home-cluster/argocd/application.yaml` в `syncPolicy.syncOptions` добавлено **`ServerSideApply=true`** — синк идёт через server-side apply, без раздувания annotations тем же механизмом.

**Если синк всё ещё падает:** проверь версию Argo CD (опция нужна с поддерживаемых релизов); запасной путь — ставить CRD ESO вне Argo (`kubectl apply --server-side -f …` из chart) и в `external-secrets/external-secrets-values.yaml` выставить `installCRDs: false`.
