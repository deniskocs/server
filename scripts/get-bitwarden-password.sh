#!/bin/bash
#
# Читает секрет (пароль или токен) из Bitwarden CLI по имени записи.
# Сначала интерактивный `bw login`, затем поиск item и вывод значения в stdout
# (удобно передавать в переменные или pipe).
#
# Параметры:
#   $1  — имя записи в Bitwarden (как в UI / поиске), например DOCKER_HUB_ACCESS_TOKEN
#
