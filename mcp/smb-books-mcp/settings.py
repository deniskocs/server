"""SMB + корень библиотеки: config.yaml или SMB_* / BOOKS_LIBRARY_ROOT."""
...
def _from_mapping(data: dict[str, Any]) -> SmbConfig:
    ...
    if not root:
        raise ValueError(
            "В конфиге нужен непустой root — каталог библиотеки на шаре (как BOOKS_LIBRARY_ROOT)"
        )
...
def load_smb_config() -> SmbConfig:
    ...
    if os.environ.get("SMB_HOST") and os.environ.get("SMB_USER"):
        lib_root = (
            (os.environ.get("SMB_ROOT") or os.environ.get("BOOKS_LIBRARY_ROOT") or "")
            .strip()
        )
        if not lib_root:
            raise ValueError(
                "Задайте SMB_ROOT или BOOKS_LIBRARY_ROOT — каталог библиотеки на шаре"
            )
        _cached = SmbConfig(
    ...
    raise FileNotFoundError(
        ...
        "или задайте SMB_HOST, SMB_USER и SMB_ROOT или BOOKS_LIBRARY_ROOT."
    )
