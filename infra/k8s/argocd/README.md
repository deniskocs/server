...
Bootstrap Argo CD (Helm): Terraform `infra/k8s/platform` (`argocd.tf` + `values/argocd.yaml`).

## Первичная регистрация (один раз)

1. После `terraform apply` в `infra/k8s/platform`.
...
2. `terraform apply` в `infra/k8s/platform` — удалит `argocd-apps` и `platform-root`.
